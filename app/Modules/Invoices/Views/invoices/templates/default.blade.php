...
<head>
    ...
    <title>{{ trans('blend.invoice') }} #{{ $invoice->number }}</title>
...
<table>
    ...
    <tr>
        <td style="width: 30%;" valign="top">
            <span class="info">{{ mb_strtoupper(trans('blend.issued')) }}:</span> {{ $invoice->issued_date_formatted }}<br>
            <span class="info">{{ mb_strtoupper(trans('blend.due_date')) }}:</span> {{ $invoice->due_date_formatted }}<br><br>
            <span class="info">{{ mb_strtoupper(trans('blend.bill_to')) }}:</span><br>
            {{ $invoice->client->name }}<br>
            @if ($invoice->client->address) {!! $invoice->client->address_formatted !!}<br>@endif
        </td>
        <td style="width: 40%; text-align: center;" valign="top">
            <h1>{{ mb_strtoupper(trans('blend.invoice')) }} #{{ $invoice->number }}</h1>
        </td>
    </tr>
</table>
...
<table class="alternate">
    ...
    <tr>
        ...
        <td class="amount">{{ $invoice->subtotal_formatted }}</td>
    </tr>

    <tr>
        <td colspan="4" class="amount">Tax</td>
        <td class="amount">{{ $invoice->tax_formatted }}</td>
    </tr>

    <tr>
        <td colspan="4" class="amount">{{ mb_strtoupper(trans('blend.total')) }}</td>
        <td class="amount">{{ $invoice->total_formatted }}</td>
    </tr>
    </tbody>
</table>
...
<table>
    ...
    <tr>
        <td></td>
        <td style="border-top: solid 1px #000">Authorized person for signing the invoice</td>
    </tr>
</table>
